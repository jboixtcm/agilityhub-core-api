...
class AuditedAspectIT extends AbstractIntegrationTest {
    ...
    void T_14_12_parameterEntryAppearsOnlyAfterOuterCommitWithCapturedMetadata() {
        ...
        assertThat(entries()).singleElement().satisfies(entry -> {
            ...
            assertThat(entry.targetType()).isEqualTo("Parameter");
            assertThat(entry.targetId()).isEqualTo("parameter-a");
            ...
            assertThat(entry.at()).isEqualTo(Instant.parse("2026-01-01T01:00:00Z"));
    ...
    @Test void T_14_07_manualNoOpIsSkippedAndReasonOnlyEntryIsKept() {
        ...
        assertThatThrownBy(() -> writer.write(command(AuditAction.CLUB_UPDATED, null, "reason"))).hasMessage("Audit target id is required");
        assertThatThrownBy(() -> writer.write(command(AuditAction.CLUB_UPDATED, " ", "reason"))).hasMessage("Audit target id is required");
        assertThatThrownBy(() -> writer.write(new AuditCommand(AuditAction.CLUB_UPDATED, null, "id", null, null, null, "reason")))
                .hasMessage("Audit target type is required");
    }
    ...
    @Test void T_14_11_requiredIndexesExistAtStartupWithoutTtl() {
        ...
        assertThat(indexes).extracting(index -> index.get("key")).contains(
                ...
                new Document("clubId", 1).append("targetType", 1).append("targetId", 1).append("at", -1));
    ...
    static class ParameterStore extends TenantRepository<AuditableParameter> implements AuditableLoader {
        ...
        @Override public String targetType() { return "Parameter"; }
        @Override public Object load(String id) { return findById(id).orElse(null); }
    }

    static class FakeService {
        private final ParameterStore parameters;
        FakeService(ParameterStore parameters) { this.parameters = parameters; }

        @Transactional
        @Audited(action = AuditAction.PARAMETER_CHANGED, targetType = "'Parameter'", target = "#id", member = "'member-a'")
        ...
        @Audited(action = AuditAction.PARAMETER_CHANGED, targetType = "'Parameter'", target = "#p0")
        public AuditableParameter withoutTransaction(String id, String value) { return replace(id, value); }

        @Transactional
        @Audited(action = AuditAction.PARAMETER_CHANGED, targetType = "#result.type")
        ...
        @Transactional
        @Audited(action = AuditAction.PARAMETER_CHANGED, targetType = "'Parameter'", target = "#id")
    ...
    static class AuditTestConfiguration {
        ...
        @Bean FakeService auditFakeService(ParameterStore store) { return new FakeService(store); }
