...
public final class IdentityResponses {
    ...
    public record Session(@Schema(requiredMode = REQUIRED, format = "uuid") String id,
            ...
            @Schema(requiredMode = NOT_REQUIRED, format = "uuid") String clubId, @Schema(requiredMode = NOT_REQUIRED) Profile activeProfile,
            @Schema(requiredMode = REQUIRED) String deviceLabel,
            @Schema(requiredMode = REQUIRED) Instant createdAt,
            @Schema(requiredMode = REQUIRED) Instant expiresAt, @Schema(requiredMode = NOT_REQUIRED) Instant lastUsedAt) { }
    ...
    public record OnboardingState(@Schema(requiredMode = REQUIRED) boolean pending,
            ...
            @Schema(requiredMode = NOT_REQUIRED) RequiredConsent requiredConsent,
    ...
    public record OnboardingField(@Schema(requiredMode = REQUIRED) String key,
            @Schema(requiredMode = NOT_REQUIRED, types = {"string", "null"}) String value,
            @Schema(requiredMode = REQUIRED) boolean required) { }

    @JsonInclude(JsonInclude.Include.NON_NULL)
    public record UserInfo(@Schema(requiredMode = REQUIRED) String sub, @Schema(requiredMode = NOT_REQUIRED) String email, @Schema(requiredMode = NOT_REQUIRED) Boolean email_verified,
            @Schema(requiredMode = NOT_REQUIRED) String name, @Schema(requiredMode = NOT_REQUIRED) String locale,
            @Schema(requiredMode = NOT_REQUIRED, description = "Included only with the memberships scope") List<UserInfoMembership> memberships) { }
    ...
    public record PublicJwk(@Schema(requiredMode = REQUIRED, allowableValues = "RSA") String kty,
            @Schema(requiredMode = REQUIRED) String kid, @Schema(requiredMode = NOT_REQUIRED) String use, @Schema(requiredMode = NOT_REQUIRED) String alg,
