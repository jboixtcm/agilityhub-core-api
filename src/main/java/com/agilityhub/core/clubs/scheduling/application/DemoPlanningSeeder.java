...
public class DemoPlanningSeeder implements DemoSeedStep {
    ...
    @Override public Map<String, Integer> apply(Input input) {
        ...
        // The week offsets generated by this run: a re-anchored run keeps every week that already exists as it is (R-06: a
        // generated or validated week is never generated again) and dates the loose classes, blocks and cancellations only
        // into the weeks it generated itself.
        var fresh = new HashSet<Integer>();
        for (var w : spec.weeks()) {
            var week = generation.create(input.weekStart().plusWeeks(w.offset())).week();
            if (input.reanchor() && (week.generatedAt() != null || week.state() == WeekState.VALIDATED)) { counts.merge("keptWeeks", 1, Integer::sum); continue; }
            ...
            counts.merge("weeks", 1, Integer::sum); counts.merge("generatedClasses", result.classCount(), Integer::sum); fresh.add(w.offset());
        ...
        java.util.function.IntPredicate dated = offset -> !input.reanchor() || fresh.contains(offset);
