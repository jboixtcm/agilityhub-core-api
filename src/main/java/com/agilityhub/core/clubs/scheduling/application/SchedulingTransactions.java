 ...
 * document may meet as one (E5-T15). The check is the shared {@link TransactionRetries#conflict}. Retries and
 * exhaustions are counted under the `scheduling` context.
 */
@Service
public class SchedulingTransactions {
    ...
    /** 50–150 ms, uniformly. */
    static long jitter() { return TransactionRetries.jitter(); }
    SchedulingTransactions(TransactionTemplate transactions, PlanningContext context, TransactionRetries retries,
            java.util.function.LongSupplier jitter, TransactionRetries.Backoff backoff) {
