...
public class DogsController {
    ...
    @ContractErrors({VALIDATION_ERROR, FILE_NOT_FOUND, FILE_TOO_LARGE, FILE_TYPE_NOT_ALLOWED, CHIP_ALREADY_EXISTS, STALE_VERSION, INVALID_STATE, MEMBER_ERASED,
    ...
    @Operation(summary = "Update dog",
            description = "Tenant comes from the JWT. ADMIN endpoints reject impersonation; erased members reject mutations. "
                    ...
                    + "GET /members/{id}/signup shows for that type keeps its file, and any other fileKey is a new file, the ADMIN's own "
                    + "POST /attachments/upload-url with purpose = DOG_DOCUMENT (R-04-19, amended 26-09); see DogPatch.documents. "
                    + "With signup.requireDogDocumentAtSignup, a VACCINATION_CARD without files answers 422 DOG_DOCUMENT_REQUIRED only when the "
                    + "request changes the card's keys (sending back a PENDING card is no change), and never when the reused dog of a pending "
                    + "readmission has its own card with a file. "
