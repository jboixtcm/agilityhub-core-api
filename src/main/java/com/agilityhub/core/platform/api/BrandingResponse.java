...
public record BrandingResponse(ClubSummary club, Theme theme, List<String> locales, String defaultLocale,
    ...
    public static BrandingResponse from(ClubConfig config) {
        ...
        return new BrandingResponse(new ClubSummary(club.slug(), club.name(), club.city(), blankToNull(club.legalName()), blankToNull(club.taxId())), club.theme(), club.locales(), club.defaultLocale(),
     ...
     * the public web to show (the footer); always present, `null` when the club has not set them.
     */
