...
public record MigrationReport(boolean dryRun, List<Entry> rows) {
    ...
    public record Entry(String file, int row, String entity, String outcome, String code) { }
    ...
    public String render() {
        ...
        for (var row : rows) { out.append(row.file()).append(':').append(row.row()).append(' ').append(row.entity()).append(' ').append(row.outcome()).append(' ').append(row.code()).append('\n'); }
        return out.toString();
    }
}
