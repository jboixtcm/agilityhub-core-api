...
4. Run every **Verification** command; put the evidence in the task's **Executor report**. **Always `./mvnw -q clean verify`** where a task says `./mvnw -q verify`: without `clean`, `jacoco.exec` keeps the coverage of earlier runs and hides a coverage failure that CI (always clean) will catch. Evidence logs under `roadmap/evidence/<ID>/` must be **committed**: `.gitignore` un-ignores `roadmap/evidence/E5-*/*.log` … `E12-*`; never add an ignore rule for them. With `-q` a green `verify` prints no summary: also commit a test summary read from `target/surefire-reports` and `target/failsafe-reports` (tests, failures, errors and skipped, per class) together with the exit code (24-09). **Evidence rule**: for every Verification command paste the exact command, its exit code and the **last 40 lines** of its output; when the output is longer, write the complete output to `roadmap/evidence/<ID>/NN-<name>.log` (committed; tokens/secrets truncated) and reference the file. A task file must stay under **120 KB** (`check.py` refuses `awaiting_verification` above that): earlier failing attempts get one line each (what failed → what you changed) plus their log file; only the final run keeps its tail in the report.
