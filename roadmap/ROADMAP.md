...
- [ ] `bin/e3-smoke` green twice on the local stack (E3-T05 evidence `10-smoke-second.log`, `11-smoke-image.log`): public signup (family group found, SEPA without IBAN → warning) → D1 pending → D2 validation → N-02 in the mailbox → welcome link → `/me`; add-dog → N-37; rejection → N-03; `signup.enabled=false` → `422 SIGNUP_CLOSED`. — **reopened 24-09**: red on `main` since E4-T05. The «future verticals» assertion is stale (E3-T07 logs `03`/`04`) → E3-T08 step 1.
- [ ] `GET /dashboard` with the seed: `pendingSignups` 3 (1 older than the warn days), `activeMembers` 184, class/training blocks `null`/0 (ports until E4/E5), `dogsByLevel` 242 over 8 levels (E3-T04/E3-T05). — **reopened 24-09**: the class block is live since E4-T05, and the chart changes with E35 → E3-T11 re-states the values.
...
- [ ] Gate audit 24-09 (`roadmap/reviews/gate-E3/consolidated.md`: fail, 1 blocker and 21 majors across both repos): E3-T08, E3-T09 and E3-T10 verified, and the E3-T11 re-run green.
